...
class TransactionValidator {
   ...
   */
  constructor(database, tempStorage, transactionSecurity) {
  ...
  validateSignatureAsync(transaction) {
    return new Promise((resolve, reject) => {
      this.transactionSecurity.verifySignatureAsync(transaction).then((isValid) => {
        if (isValid) {
          resolve();
        } else {
...
module.exports = {
   ...
   */
  getInstance(database, tempStorage, transactionSecurity) {
    if (!instance) {
      if (!database || !tempStorage || !transactionSecurity) {
        throw new Error('Invalid argument(s)');
      }
      instance = new TransactionValidator(database, tempStorage, transactionSecurity);
    }
    return instance;
  },
};
