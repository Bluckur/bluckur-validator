...
class BlockValidator {
   ...
   */
  constructor(database, blockSecurity) {
  ...
  validateHashAsync(block) {
    return new Promise((resolve, reject) => {
      this.blockSecurity.getHashAsync(block).then((hash) => {
        if (block.hash === hash) {
          resolve();
        } else {
...
module.exports = {
   ...
   */
  getInstance(database, blockSecurity) {
    if (!instance) {
      if (!database || !blockSecurity) {
        throw new Error('Invalid argument(s)');
      }
      instance = new BlockValidator(database, blockSecurity);
    }
    return instance;
  },
};
