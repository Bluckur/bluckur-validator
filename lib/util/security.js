...
S.sign = function sign(message, privateKey) {
...
S.signDetached = function signDetached(transaction, privateKey) {
  return new Promise((resolve, reject) => {
    if (!transaction || !privateKey) {
      ...
      naclFactory.instantiate((nacl) => {
        try {
          ...
          const messageBytes = this.messageToBytes(JSON.stringify(transaction));
          // Sign message and package up into packet
...
S.verify = function verify(signature, pubKey) {
  return new Promise((resolve, reject) => {
    if (!signature || !pubKey) {
      reject(new Error('invalid message'));
    } else {
