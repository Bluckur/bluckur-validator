...
T.verifyAmount = function verifyAmount(transaction, pendingTransactions, state) {
  if (this.checkTransactionArguments(transaction)) {
    let remainingState = state - transaction.amount;
    pendingTransactions.forEach((pendingTransaction) => {
      if (pendingTransaction.sender === transaction.sender) {
        remainingState -= pendingTransaction.amount;
      }
    });
    return remainingState >= 0 ? true : new Error('Not enough balance to send the transaction');
  }
