 ...
 */
T.verifyAmount = function verifyAmount({ sender, amount }, pendingBlock, globalState) {
  if (this.checkTransactionArguments(transaction)) {
      let remainingBalance = globalState[sender];
      remainingBalance -= amount;
      pendingBlock.transactions.forEach((pendingTransaction) => {
        if (pendingTransaction.sender === sender) {
