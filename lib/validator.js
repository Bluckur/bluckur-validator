...
class Validator {
  ...
  onBlockchainReply({ data: receivedBlocks }) {
    ...
    const lastReceivedBlockHeader = receivedBlocks[receivedBlocks.length].blockHeader;
    if (this.blockchainValidator.validate(receivedBlocks)) {
      this.database.getBlockchainAsync().then((blocks) => {
        const lastBlockHeader = blocks[blocks.length].blockHeader;
        // Delete and rebuild blockchain if it's bad!
        if (lastBlockHeader.hash !== lastReceivedBlockHeader.blockHeader.hash) {
          this.database.deleteBlocksAsync(0, lastBlockHeader.length - 1).then(() => {
            return this.database.putBlocksAsync(blocks);
          }).catch((err) => {
            console.log(err);
          });
        }
        this.database.clearGlobalState().then(() => {
          let transactions = [];
