...
const BlockSecurity = require('./security/blockSecurity');
const BlockValidator = require('./validators/blockValidator');
const TransactionValidator = require('./validators/transactionValidator');
const GenesisBlockBuilder = require('./services/genesisBlockBuilder');
...
class Validator {
  constructor() {
    ...
    this.tempStorage = TemporaryStorage.getInstance();

    // Security services
    this.security = Security.getInstance();
    this.transactionSecurity = TransactionSecurity.getInstance(this.security);
    this.blockSecurity = BlockSecurity.getInstane(this.security);

    // Validators
    this.blockValidator = BlockValidator.getInstance(this.database, this.blockSecurity);
    this.transactionValidator = TransactionValidator.getInstance(this.database, this.tempStorage, this.transactionSecurity);

    this.genesisBlockBuilder = GenesisBlockBuilder.createInstance(this.blockSecurity, this.transactionSecurity);
  }
}
